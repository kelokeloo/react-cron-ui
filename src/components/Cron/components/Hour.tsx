...
export const Hour = (props: Props) => {
  ...
  const [type, setType] = useState<HourEnum>(HourEnum.EVERY);
  ...
  const radioInfoList = [
    {
      ...
      component: (
        <Every
          ref={(refObj) => {
            if (refObj) {
              registerHandler(HourEnum.EVERY, refObj.getValue);
            }
          }}
          unit={unit}
          onChange={(value) => {
    ...
    },
    {
      ...
      component: (
        <Range
          ref={(refObj) => {
            if (refObj) {
              registerHandler(HourEnum.RANGE, refObj.getValue);
            }
          }}
          unit={unit}
    ...
    {
      key: HourEnum.INTERVAL,
      label: "间隔",
      component: (
        <Interval
          ref={(refObj) => {
            if (refObj) {
              registerHandler(HourEnum.INTERVAL, refObj.getValue);
            }
          }}
          unit={unit}
    ...
    {
      key: HourEnum.SPECIFIC,
      label: "指定",
      component: (
        <Specific
          ref={(refObj) => {
            if (refObj) {
              registerHandler(HourEnum.SPECIFIC, refObj.getValue);
            }
          }}
  ...
  return (
    <div>
      <Radio.Group
        value={type}
        onChange={(e) => {
          const type = e.target.value as HourEnum;
          setType(type);
