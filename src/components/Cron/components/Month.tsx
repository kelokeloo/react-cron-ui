...
import {
  everyRegex,
  monthIntervalRegex,
  monthSpecificRegexSingle,
  monthSpecificRegexMultiple,
  monthRangeStrRegex,
  rangeRegex,
...
export const Month = (props: Props) => {
  ...
  const parserMergedValue = (raw: string) => {
    ...
    } else if (monthIntervalRegex.test(raw)) {
      const match = raw.match(monthIntervalRegex)!;
    ...
    } else if (monthSpecificRegexSingle.test(raw)) {
      const match = raw.match(monthSpecificRegexSingle)!;
      if (isMonthStr(match[1])) {
    ...
    } else if (monthSpecificRegexMultiple.test(raw)) {
    ...
    } else if (monthRangeStrRegex.test(raw)) {
      const match = raw.match(monthRangeStrRegex)!;
      defaultSplitValueRef.current.range = {
